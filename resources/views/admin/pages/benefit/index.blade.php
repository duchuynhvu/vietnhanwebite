...
@section('page')
    <div class="page">
        <div class="page-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ __('admin.benefit') }}</li>
            </ol>
            <div class="page-header-actions">
                <a class="btn btn-sm btn-default btn-outline btn-round" href="{{ url('/admin/benefits/create') }}">
                                    ...
                                    <table class="table table-bordered" data-plugin="selectable"
                                        ...
                                        @foreach ($benefits as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ ($benefits->currentpage()-1) * $benefits->perpage() + $key + 1 }}</td>
                                                <td style="background: #f28e20" width="100"><a
                                                            href="{{ asset('/uploads/benefits') }}/{{ $item->icon }}"
                                                            class="inline-block" data-plugin="magnificPopup"><img
                                                                width="100" height="100"
                                                                class="preview"
                                                                src="{{ asset('/uploads/benefits') }}/{{ $item->icon }}"></a>
                                                </td>
                                                <td>{{ $item->title }}</td>
                                                @if ($item->feature == 1)
                                                    <td>
                                                        <input class="feature-switch" type="checkbox"
                                                               data-plugin="switchery" value="{{ $item->id }}" checked/>
                                                    </td>
                                                @else
                                                    <td>
                                                        <input class="feature-switch" type="checkbox"
                                                               data-plugin="switchery" value="{{ $item->id }}"/>
                                                    </td>
                                                @endif
                                                <td>{{ $item->created_at }}</td>
                                                <td>{{ $item->updated_at }}</td>
                                                <td><a class="btn btn-default"
                                                       href="{{ url('/admin/benefits') }}/{{ $item->id }}/edit">{{ __('admin.edit') }}</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
    ...
    <input type="hidden" id="database" name="_token" value="/admin/benefits">
@endsection