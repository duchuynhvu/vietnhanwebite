...
@section('page')
    <div class="page">
        <div class="page-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ __('admin.regent') }}</li>
            </ol>
            <div class="page-header-actions">
                <a class="btn btn-sm btn-default btn-outline btn-round" href="{{ url('/regents/create') }}">
                                    ...
                                    <table class="table table-bordered" data-plugin="selectable" data-row-selectable="true">
                                        ...
                                        @foreach ($regents as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ ($regents->currentpage()-1) * $regents->perpage() + $key + 1 }}</td>
                                                <td><a href="{{ asset('/uploads/regents') }}/{{ $item->image }}"
                                                       class="inline-block" data-plugin="magnificPopup"><img
                                                                class="preview"
                                                                src="{{ asset('/uploads/regents') }}/{{ $item->image }}"></a>
                                                </td>
                                                <td>{{ $item->name }}</td>
                                                <td>{{ $item->regency }}</td>
                                                ...
                                                <td>
                                                    <a class="btn btn-default"
                                                       href="{{ url('/regents') }}/{{ $item->id }}/edit">{{ __('admin.edit') }}</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
    ...
    <input type="hidden" id="database" name="_token" value="/regents">
@endsection